...
public interface DocumentParser {
    ...
    /**
     * 兼容旧解析 API。新链路应优先使用 {@link #parseStructured(byte[], String, Map)}。
     */
    default ParseResult parse(byte[] content, String mimeType, Map<String, Object> options) {
        ParsedDocument parsed = parseStructured(content, mimeType, options);
        return ParseResult.of(BlockTextRenderer.render(parsed.blocks()), parsed.metadata());
    }

    /**
     * 兼容旧流式文本提取 API。新链路应先读取字节后调用结构化解析。
     */
